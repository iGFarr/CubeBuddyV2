...
struct GameView: View {
    ...
    var body: some View {
        SceneView(
            ...
            options: [.autoenablesDefaultLighting, .rendersContinuously], delegate: CBSceneDelegate())
            .gesture(
                fingerDrag
            )
    }
...
@MainActor class CBSceneDelegate: NSObject, SCNSceneRendererDelegate {
    let settings = SharedView.shared
...
class GameScene: UIView {
    ...
    func changeCamera() {
        ...
        cameraOrbit.eulerAngles.y = Float(-Float.pi) * scrollWidthRatio + dy
...
class CustomNode: SCNNode {
    ...
    init(tip:Tip, geometry: SCNGeometry) {
        super.init()
        self.tip = tip
